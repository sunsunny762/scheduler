/*
 ...
 Date: 19/02/2026 15:29:38
*/


-- ----------------------------
-- procedure structure for spCertificationBlueAwardCount_Get
-- ----------------------------
IF EXISTS (SELECT * FROM sys.all_objects WHERE object_id = OBJECT_ID(N'[portal].[spCertificationBlueAwardCount_Get]') AND type IN ('P', 'PC', 'RF', 'X'))
	DROP PROCEDURE [portal].[spCertificationBlueAwardCount_Get]
GO

CREATE PROCEDURE [portal].[spCertificationBlueAwardCount_Get]
  
AS
BEGIN

    -- Blue award submissions (NCZ Form) from Portal without "Certification issued" status
    SELECT  count(*) as cnt
