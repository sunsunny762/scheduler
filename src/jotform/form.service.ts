...
@Injectable()
export class FormService { // For Portal
  ...
  /*public async processJotformResponses_NOT_USED(): Promise<void> {
    try { 
      const query = await this.databaseService.execute(process.env.JOTFORM_PROCESS_LOCAL_SP || "[Forms].[spFormResponse_SelectToProcess_NonPortal]"); //"[Forms].[spFormResponse_SelectToProcess]");
  ...
  public async processPortalFormResponses(): Promise<void> {
    // Jotforms submissions made through Portal, (Blue Award is not included in this function)
    try { 
      const query = await this.databaseService.execute(process.env.JOTFORM_PROCESS_LOCAL_SP || "[Forms].[spFormResponse_SelectToProcess_Portal]");
      const responses = query.results;

      console.log(`Total Portal submissions to process : ${responses.length}`)
      let processResult: ProcessingResponse = ProcessingResponse.unknown;
      for (let response of responses) {
        try {
          this.errorLoggerService.setFormSubmissionIds(response.jotFormId, response.submissionId);
          
          console.log(`Processing Portal SubmissionId: ${response.submissionId}`);

          if (response.categoryId == FormCategory.GoldAwardQuestionnaire) {
            processResult = await this.processGoldAwardForm(response);
          }
          else if (response.categoryId == FormCategory.BlueAwardQuestionnaire) { // Blue Award will be processed through another function
            processResult = await this.processSilverAwardForm(response);
          }
          else {
            processResult = await this.processResponse(response);
          }
          ...
          console.error("processJotformResponses_Portal1",e);
        }
        finally {
          // Mark as processed so won't be picked again
          //if (processResult !== ProcessingResponse.entity_not_found) {
          if(processResult == ProcessingResponse.success) {
            try{
              ...
              console.error("processJotformResponses_Portal2", e);
            }
          }
        }
      }
      ...
      console.error("processJotformResponses_Portal3",e);
    }
  }
  ...
  private processCustomJson(key, obj) {
    if (key.indexOf('BAQ_ReportStartDate') > -1) {
      ...
      for (let index = 0; index < data.length; index++) {
        ...
        obj[`BAQ_CompanyVehiclesMileage_${element}`] = data[index]['Total distance'] * conversionFactor;
      }
      delete obj[key];
    }
  ...
  public async processSilverAwardForm(response: any) {
    try { // Blue submission will not come through, there will be different process for it
      ...
      //Create dimension submission record.
      const submissionDate = jsonObj['buildDate'] ? Math.round(+jsonObj['buildDate'] / 1000) : undefined;
      const dimSubmissionId = await this._createSubmission(response.submissionId, submissionDate, 0, 1, response.companyId, response.entityTypeId, response.formId);
      if (!dimSubmissionId) {
      ...
      /* // ***** Commented below code as it is not required
         // No need to create  Company, Person etc tasks and relationships, Certification (Already created on Certification create from Portal)
         // Not required to set BAQ uniqueId or BAQ uniqueId custom field is set in Certification task
  ...
  public async processResponse(response: any) {
    try {
      ...
      let entityId = response.companyId; // Portal company id
      const certTaskId = response.certTaskId; // Clickup certification task id
      ...
      //     const response = await this.clickupService.createTask(formName, companyName, taskDetails.status, certTaskId, taskDetails.listId, jotformId, jotformSubmissionId);
      //     if (!response?.id) {
      ...
      if (process.env.CLICKUP_CREATE_SUBTASK == '1' && formId != 11) { // Don't create subtask for C&HW submission
        //Create Subtask in clickup.
        const taskResponse = await this.clickupService.getTaskDetailsFromDB(certTaskId);
        if (taskResponse.success) { 
          const taskDetails = taskResponse.data[0];
  ...
  private _getGreenTariffPercent(questions: any, data: any) {
    let tariffYesNo: Question = questions.find((x: Question) => x.questionTypeId == QuestionType.electricity_greenTariffYesNo_portal);
    if (tariffYesNo) {
      const hasGreenTarrif = data[tariffYesNo.reference];
      if (!hasGreenTarrif || hasGreenTarrif != "Yes") return null;

      let tariffPercent: Question = questions.find((x: Question) => x.questionTypeId == QuestionType.electricity_greenTariffPercentage_portal);
      if (tariffPercent) {
        const greenTariffPercent = data[tariffPercent.reference];
